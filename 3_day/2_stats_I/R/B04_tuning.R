...
# with Maipo_fields.Rdata data file:
setwd("C:/Users/yo54pow/Nextcloud/Teaching/Geo408-w2018/R")
                             ...
                             maxdepths = c(1,2,3,5,7,10,15,20,25,30))
{
  ...
  for (i in 1:length(maxdepths)) {
    ...
    # objects!
    res <- sperrorest(formula = formula, data = data, coords = c("utmx","utmy"),
                      model_fun = rpart, model_args = list(control=ctrl), 
                      pred_args = list(type = "class"), 
                      smp_fun = partition_kmeans,
...
task <- makeClassifTask(data = d2, target = "croptype", 
                        coordinates = xy)
task
...
out <- mlr::resample(learner = lrnr, task = spatial.task,
               resampling = resampling, measures = list(acc))
mean(out$measures.test$acc)


# use random search to optimize hyperparameter:
tnr <- makeTuneControlGrid(resolution = 10)
# Note that 'resolution' is a misnomer! This is the number of steps.
...
selftuning_lrnr <- makeTuneWrapper(learner = lrnr, 
...
result <- mlr::resample(learner = selftuning_lrnr, 
                       task = task,
