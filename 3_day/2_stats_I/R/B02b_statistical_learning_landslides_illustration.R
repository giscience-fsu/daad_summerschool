...
mypredict2d <- function(fit,
                        ...
                        data = d,
                        newdata = list(
                          plancurv = 0, profcurv = 0, cslope = 20,
...
# Plot prediction map
library(raster)
glm.raster <- raster(here("R/data/glmpred_simple.asc"))
plot(glm.raster)
...
# Plot prediction map
library(raster)
gam.raster <- raster(here("R/data/gampred_simple.asc"))
plot(gam.raster)
...
library(rpart)
fit <- rpart(slides89 ~ slope + distroad,
  data = d,
...
fit <- randomForest(slides89 ~ slope + distroad,
  data = d,
...
multi.local.function(
  in.grids = c("slope", "distroad"),
  out.varnames = paste("rfpred_simple", sep = ""), path = here("R/data"),
  fun = grid.predict, control.predict = list(type = "prob"),
  fit = fit, trafo = my.trafo.simple, predict.column = "TRUE",
  quiet = FALSE
)

rf.raster <- raster(here("R/data/rfpred_simple.asc"))
plot(rf.raster)
...
fit <- svm(slides89 ~ slope + distroad,
  data = d,
...
multi.local.function(
  in.grids = c("slope", "distroad"),
  out.varnames = paste("svmpred_simple", sep = ""), path = here("R/data"),
  fun = grid.predict, predfun = mypredictsvm, control.predict = list(),
  fit = fit.svm, trafo = my.trafo.simple,
  quiet = FALSE
)

svm.raster <- raster(here("R/data/svmpred_simple.asc"))
plot(svm.raster)

# Compare several hyperparameters:
fit.svm.01.01 <- svm(slides89 ~ slope + distroad,
  data = d,
  type = "C-classification", kernel = "radial",
  probability = TRUE,
  cost = 0.1, gamma = 0.1
)
fit.svm.01.1 <- svm(slides89 ~ slope + distroad,
  data = d,
  type = "C-classification", kernel = "radial",
  probability = TRUE,
  cost = 0.1, gamma = 1
)
fit.svm.1.01 <- svm(slides89 ~ slope + distroad,
  data = d,
  type = "C-classification", kernel = "radial",
  probability = TRUE,
  cost = 1, gamma = 0.1
)
fit.svm.1.1 <- svm(slides89 ~ slope + distroad,
  data = d,
  type = "C-classification", kernel = "radial",
  probability = TRUE,
  cost = 1, gamma = 1
)
fit.svm.10.1 <- svm(slides89 ~ slope + distroad,
  data = d,
  type = "C-classification", kernel = "radial",
  probability = TRUE,
  cost = 10, gamma = 1
)
fit.svm.1.10 <- svm(slides89 ~ slope + distroad,
  data = d,
  type = "C-classification", kernel = "radial",
  probability = TRUE,
  cost = 1, gamma = 10
)
fit.svm.10.10 <- svm(slides89 ~ slope + distroad,
  data = d,
  type = "C-classification", kernel = "radial",
  probability = TRUE,
  cost = 10, gamma = 10
