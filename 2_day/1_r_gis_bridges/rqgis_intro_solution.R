# Filename: intro_rqgis.R (2019-01-08)
#
# TO DO: Introduce RQGIS3
#
...
params = get_args_man("native:intersection")
...
params$OVERLAY = poly_2
params$OUTPUT = file.path(tempdir(), "out.shp")
int = run_qgis("native:intersection", params = params, load_output = TRUE)
# visualize it
...
# SAGA algorithms work under Windows both in the ltr and the QGIS dev version
# and under Linux when using the QGIS LTR (3.4.10)
# however, some SAGA algorithms (e.g., saga:sagawetnessindex) seem not to work under
# QGIS 3.8 (Linux)
...
twi = run_qgis(alg = alg,
               ...
               load_output = TRUE)
# here, we are only interested in the wetness index
twi = twi$TWI
